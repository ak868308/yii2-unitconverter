...
namespace ak868308\unitconverter;
...
/**
 * This file is part of the Convertor package (i.e Unit.php)
 ...
 * 
 * Usage: 
 * $unitConvertor = new UnitConvertor(10, "m");
 * $unitConvertor->to("ft"); //returns converted feet value
 * 
 * using another file somewhere in the project:
 * $c=new UnitConvertor(100,"mps",'/path/to/my/own/Units.php');
 * 
 * //using the name of the file in conf:
 * $c=new UnitConvertor(100,"mps",'BaseUnits.php');
...
class UnitConverter extends Component {
    ...
    /**
     * value to convert
     * @var integer|float|double|NULL 
     */
    private $value = null;

    /**
     * The base units that are used as multipliers for smallest possible unit from measured unit
     * base unit of value    
     * @var string|boolean 
     */
     ...
     * @param $unitFile - either the filename in folder OR a path to another file that exists.
     * @throws FileNotFoundException if the file does not exist.
     */
    protected function defineUnits($unitFile) {
        ...
        if (file_exists($unitFile))
            ...
            throw new NotFoundHttpException("File could not be found. Given path='{$unitFile}' either use the name of one of the pre defined configuration files or pass the complete path to the file.");
    }
     ...
     * @throws Exception - specific invalid unit exception
     */
    public function from($value, $unit) {
        ...
        if ($unit) {
            ...
            if (array_key_exists($unit, $this->units)) {
                ...
                throw new InvalidConfigException("Conversion from Unit u=$unit not possible - unit does not exist.");
            }
        } else {
            $this->value = $value;
        }
     ...
     * @param    int $decimals (optional, default-2) - the decimal precision of the conversion result
     * @param    boolean $round (optional, default-false) - round or floor the conversion result
     * @return   mixed
     */
    public function to($unit, $decimals = 2, $round = false) {
        ...
        if (!$unit) {
            throw new InvalidArgumentException("Unit Not Set");
        }
            ...
            if (array_key_exists($unit, $this->units)) {
                ...
                if ($this->baseUnit) {
                    if ($unitLookup["base"] != $this->baseUnit) {
                        throw new InvalidConfigException("Cannot Convert Between Units of Different Types");
                    }
                } else {
                    $this->baseUnit = $unitLookup["base"];
                }
                ...
                throw new InvalidConfigException("Conversion to Unit u=$unit not possible - unit does not exist.");
            }
        }
    }
     ...
     */
    private function toMany($unitList = [], $decimals = 2, $round = false) {
        $resultList = [];
     ...
     */
    public function toAll($decimals = 2, $round = false) {
    ...
    public function removeUnit($unit) {
        ...
        if (array_key_exists($unit, $this->units)) {
            ...
            throw new Exception("Removal of Unit u={$unit} not possible - unit does not exist.");
        }
    }
    ...
    public function getUnits($unit) {
        ...
        if (array_key_exists($unit, $this->units)) {
            ...
            throw new Exception("Unit u=$unit Does Not Exist");
        }
    }
